...
const Motos = async () => {
	const vehicules = await getByMotoPath("/motos/model");
								...
								<Suspense fallback={<p>Chargement des motos ...</p>}>
									{vehicules.map((car) => (
										<CardCar
											...
											type={car.type}
											price={car.price}
											imgFront={`/pictures/moto/${car.marque?.slug}/${car.slug}.webp`}
											imgBack={`/pictures/moto/${car.marque?.slug}/${car.slug}_a.webp`}
											isNew={car.isNew}
										/>
									))}
								</Suspense>

...
const AutosType = async ({ params }) => {
	const { type } = await params;
	const vehicules = await getByMotoPathByMarque(`/autos/type/${type}`);

	const marqueVL = type.charAt(0).toUpperCase() + type.slice(1);
							...
							<div className="mx-auto px-4 py-16 sm:px-6 sm:py-24 lg:px-8 flex flex-wrap w-full flex-row items-center justify-between text-center">
								...
								<Suspense fallback={<p>Chargement des véhicules ...</p>}>
									{vehicules.map((auto) => (
										<CardCar
											key={auto._id}
											marque={auto.marque?.marque}
											model={auto.model}
											type={auto.type}
											price={auto.price}
											imgFront={`/pictures/auto/${auto.marque?.slug}/${auto.slug}.webp`}
											imgBack={`/pictures/auto/${auto.marque?.slug}/${auto.slug}_a.webp`}
											isNew={auto.isNew}
										/>
									))}
								</Suspense>
